...
<div class="page-header">
    <h1>Add Subject</h1>
    <p>Select the college, department, course, and class year, then define the subject.</p>
</div>
...
<div class="form-card" style="max-width:700px;">
        ...
        <div class="form-group">
            ...
            <select id="college_id" name="_college_id">
                ...
                @foreach($colleges as $col)
                    <option value="{{ $col->id }}" {{ old('_college_id') == $col->id ? 'selected' : '' }}>
                        {{ $col->name }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="form-grid">
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g. OJT / CEP / Data Structures" required>
        ...
        <div class="form-group">
            ...
            <input type="text" id="code" name="code" value="{{ old('code') }}" placeholder="e.g. CS301">
    ...
    <div class="form-group" style="margin-top:18px;">
        ...
        <div class="type-pills">
            @foreach(['theory' => '📖 Theory', 'practical' => '🔬 Practical', 'elective' => '⭐ Elective'] as $val => $label)
                <label class="type-pill{{ old('type', 'theory') === $val ? ' selected-'.$val : '' }}" id="pill-{{ $val }}">
                    <input type="radio" name="type" value="{{ $val }}" {{ old('type', 'theory') === $val ? 'checked' : '' }}>
                    {{ $label }}
                </label>
            @endforeach
        </div>
...
(function () {
    // ── Raw data from PHP ──
    const allDepts   = @json($departments);   // [{id, college_id, name}, ...]
    const allCourses = @json($courses);       // [{id, department_id, name, short_name}, ...]
    const allClasses = @json($classes);       // [{id, course_id, name, label}, ...]
    ...
    function populate(select, items, valField, labelFn, emptyLabel) {
    ...
    selCollege.addEventListener('change', function () {
        ...
        const depts = cid ? allDepts.filter(d => String(d.college_id) === cid) : [];
        populate(selDept,    depts,   'id', d => d.name, '— Select Department —');
    ...
    selDept.addEventListener('change', function () {
        ...
        const courses = did ? allCourses.filter(c => String(c.department_id) === did) : [];
        populate(selCourse, courses, 'id', c => (c.short_name || c.name), '— Select Course —');
        populate(selClass,  [],      'id', cl => cl.name, '— Select Class —');
    });

    selCourse.addEventListener('change', function () {
        const cid = this.value;
        const classes = cid ? allClasses.filter(cl => String(cl.course_id) === cid) : [];
        populate(selClass, classes, 'id', cl => (cl.label || cl.name), '— Select Class —');
    });
    ...
    document.querySelectorAll('.type-pill input[type=radio]').forEach(radio => {
        radio.addEventListener('change', function () {
            ...
            if (this.checked) {
                this.parentElement.classList.add('selected-' + this.value);
            }
        });
    });
})();
